...
// ---- batch streaming API (server -> client sink) ----
// One batch of exported traces sent to the callback.
...
// Callback signature to receive batches.
type BatchCallback func(BatchSnapshot)

// Option to enable batch mirroring. Use in NewHTTPClient(..., &WithBatchMirror{...}).
...
// Converts evicted internal entries into exported traces and emits one batch.
// This is called by evictingMap when it evicts entries due to capacity.
func (c *HTTPClient) flushEvictedBatch(batch []finalizedTrace) {
	if len(batch) == 0 || c.batchCb == nil {
		return
	}
	...
	for _, e := range batch {
		...
		case "dials":
			...
			case *dial:
				snap.Dials = append(snap.Dials, v.DialTrace)
			case DialTrace:
				snap.Dials = append(snap.Dials, v)
			case *DialTrace:
				snap.Dials = append(snap.Dials, *v)
			}
		case "httpReqs":
			switch v := e.Value.(type) {
			case *HTTPReqTrace:
				snap.HTTPReqs = append(snap.HTTPReqs, *v)
			case HTTPReqTrace:
				snap.HTTPReqs = append(snap.HTTPReqs, v)
			}
		case "dnsQueries":
			switch v := e.Value.(type) {
			case *DNSQueryTrace:
				snap.DNSQueries = append(snap.DNSQueries, *v)
			case DNSQueryTrace:
				snap.DNSQueries = append(snap.DNSQueries, v)
			}
		case "tlsTuns":
			switch v := e.Value.(type) {
			case *tlsTun:
				snap.TLSTunnels = append(snap.TLSTunnels, v.TLSTunnelTrace)
			case TLSTunnelTrace:
				snap.TLSTunnels = append(snap.TLSTunnels, v)
			case *TLSTunnelTrace:
				snap.TLSTunnels = append(snap.TLSTunnels, *v)
			}
		case "connections":
			switch v := e.Value.(type) {
			case *connection:
				if v.proto == syscall.IPPROTO_TCP {
					tcp := TCPConnTrace{
						TraceID:          v.ID,
						FromDial:         v.DialID,
						FromResolver:     v.FromResolver,
						HandshakeBeginAt: v.SockCreatedAt,
						HandshakeEndAt:   v.ConnectedAt,
						Connected:        !v.ConnectedAt.Undefined(),
						ConnCloseAt:      v.ClosedAt,
						AddrTuple:        v.addrTuple.toExportedAddrTuple(),
						Reused:           v.Reused,
						TotalSentBytes:   v.TotalSentBytes,
						TotalRecvBytes:   v.TotalRecvBytes,
						Conntract:        conntrackToExportedEntry(v.Conntrack.flow, v.Conntrack.capturedAt),
					}
					if c.withSockTrace {
						tcp.SocketTrace = &SocketTrace{SocketOps: v.SocketOps}
					}
					snap.TCPConns = append(snap.TCPConns, tcp)
				} else if v.proto == syscall.IPPROTO_UDP {
					udp := UDPConnTrace{
						TraceID:        v.ID,
						FromDial:       v.DialID,
						FromResolver:   v.FromResolver,
						SocketCreateAt: v.SockCreatedAt,
						ConnCloseAt:    v.ClosedAt,
						AddrTuple:      v.addrTuple.toExportedAddrTuple(),
						TotalSentBytes: v.TotalSentBytes,
						TotalRecvBytes: v.TotalRecvBytes,
					...
					if c.withSockTrace {
						udp.SocketTrace = &SocketTrace{SocketOps: v.SocketOps}
					}
					snap.UDPConns = append(snap.UDPConns, udp)
				}
			case TCPConnTrace:
				snap.TCPConns = append(snap.TCPConns, v)
			case *TCPConnTrace:
				snap.TCPConns = append(snap.TCPConns, *v)
			case UDPConnTrace:
				snap.UDPConns = append(snap.UDPConns, v)
			case *UDPConnTrace:
				snap.UDPConns = append(snap.UDPConns, *v)
			}
		}
